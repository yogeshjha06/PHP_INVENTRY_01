...
<div class="container">
  <table class="center">
      ...
      <tr>
        <td><label for="bb">Brand Name</label></td>
        <td>
        ...
        <tr>
        <td colspan="2"><button type="submit" name="submit">Sale</button></td>
        </tr>
...
if(isset($_POST['submit']))
        ...
        {
                $query="INSERT INTO `sale`(`brand`, `item`, `amount`, `uid`, `cid`, `date`, `time`) VALUES ('$brand','$item','$amount','$id','$client','$date','$time')";
...
<?php
      $sql="SELECT * FROM sale WHERE uid = '$id'";
      $result=$con->query($sql);
      $no=1;
      while($row=$result->fetch_assoc())
      {
        ...
        <tr>
          ...
          <td>".$row["date"]."</td>
          <td>".$row["time"]."</td>
          <td> <button><a href='http://localhost/nimo/saledel.php?id=$row[id]'>Delete</a></button></td>       
          <td> <button><a href='http://localhost/nimo/saleins.php?id=$row[id]&email=$email'>Edit</a></button></td>  
        </tr>
        ";
        $no++;
      }
