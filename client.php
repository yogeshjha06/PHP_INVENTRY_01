...
<div class="container">
  <table class="center">
        ...
        <td><input type="text" placeholder="Add Client" name="client" ></td>
        </tr>
...
if(isset($_POST['submit']))
        ...
        {
                $query="INSERT INTO `client`(`client`,`date`, `time`,`uid`) VALUES ('$client','$date','$time','$id');";
...
<?php
      $sql="SELECT * FROM client WHERE uid = '$id'";
      $result=$con->query($sql);
      $no=1;
      while($row=$result->fetch_assoc())
      {
        ...
        <tr>
          ...
          <td>".$row["date"]."</td>
          <td>".$row["time"]."</td>
          <td> <button><a href='http://localhost/nimo/clientdel.php?id=$row[id]'>Delete</a></button></td>       
          <td> <button><a href='http://localhost/nimo/clientins.php?id=$row[id]&email=$email'>Edit</a></button></td>  
        </tr>
        ";
        $no++;
      }
